...
Rprof("inst/r-prof-out/prof_sim_200nds.out")
# run the simulations over the network
simres = disnet_simulate(sim_input = for_sim, sim_output_dir = NA, nsims=100)
Rprof(NULL)
saveRDS(ctr$value(), "inst/r-prof-out/ctr_sim_200nds.RDS")

# *** Prof summary--------------------------------------------------------------

summaryRprof("inst/r-prof-out/prof_sim_200nds.out")
